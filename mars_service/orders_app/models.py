...
class DeviceInField(models.Model):
    ...
    device = models.ForeignKey(Device, on_delete=models.RESTRICT, verbose_name="Оборудование")
...
def status_validator(order_status):
    """Валидатор статуса заявки"""
    allowed_statuses = ["open", "closed", "in progress", "need info"]
...
class Order(models.Model):
    ...
    device_in_field = models.ForeignKey(DeviceInField, verbose_name="Оборудование", on_delete=models.RESTRICT)
    ...
    def __str__(self):
        return f"Заявка #{self.id} - {self.order_status}"